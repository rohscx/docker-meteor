        ...
        return (
          <div key={data["_id"]} style= {divStyles}>
            <Row className="show-grid" style={rowStylesMain}>
              <Col xs={8} sm={6} md={4}>{data.siteData.dataObj.hostname}</Col>
              <Col xs={6} sm={6} md={3}>{data.siteData.dataObj.role}</Col>
              <Col xs={6} sm={6} md={6}>Updated @ UTC {data.siteData.dataObj.lastUpdated}</Col>
            </Row>
            <Row className="show-grid">
              <Col xs={5} sm={6} md={3}><a href={sshLinkGen(mgmtIpAddress)}>{mgmtIpAddress}</a></Col>
              <Col xs={6} sm={6} md={3}>{reachCheck(status)}</Col>
              <Col xs={6} sm={6} md={2}>Ver: {data.siteData.dataObj.softwareVersion}</Col>
              <Col xs={6} sm={6} md={4}>Up Time: {data.siteData.dataObj.upTime}</Col>
              <Col xs={6} sm={6} md={2}>Int#: {data.siteData.dataObj.interfaceCount}</Col>
