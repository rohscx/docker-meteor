      ...
      let transTimeSeconds = (dataSize, dataType, portSpeed)=>{
        switch(dataType){
          case "MB":
            ...
            if (inSeconds <= 60){
              return (<td>{NaNCheck ? "n/a" : Math.ceil(inSeconds)} {NaNCheck ? "n/a" : " Seconds"}</td>)
            } else if (inSeconds <= 60){
              return (<td>{NaNCheck ? "n/a" : Math.ceil(inSeconds/60)}{NaNCheck ? "n/a" : " Minutes"}</td>)
            } else if (inSeconds <= 3600){
              return (<td>{NaNCheck ? "n/a" : Math.ceil(inSeconds/3600)}{NaNCheck ? "n/a" : " Hours"}</td>)
            } else if (inSeconds >= 86400 || inSeconds > 3600){
              return (<td>{NaNCheck ? "n/a" : Math.ceil(inSeconds/86400)}{NaNCheck ? "n/a" : " Days"}</td>)
            }
            break;
          default:
        }
